...
for METHOD in "Laplace" "Gaussian"; do
    for EPS in 0.1 0.2 0.5 1.0 2.0; do
        ...
        echo "Running ECG privatization (unbounded sinc) with ${METHOD} noise and eps = ${EPS}"
        python ECG.py $METHOD $EPS -1 > results/cmd.log
        mv results/cmd.log results/ECG/ECG_${EPS}_unbounded/ECG_${METHOD}_${EPS}_cmd.log
