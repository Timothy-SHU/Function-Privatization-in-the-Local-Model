...
solver = adaptive_poly_approx(func = func, interval = (0, 100), basis = 'Polynomial', degree = 2)
...
solver.solve(func, 0.5, method = 'Laplace')
plot_1D_1D(solver)
plt.subplot(2, 2, 2)
...
for scale in [1, 10, 100]:
    ...
    for i in range(10):
        solver.solve(func, method = 'Laplace')
        err_ls_sum += solver.eval(type = 'Approx')
        err_priv_sum += solver.evalPrivLoss()
        err_total_sum += solver.eval(type = 'Priv')
        solver.clear()
