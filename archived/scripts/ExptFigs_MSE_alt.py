...
def getStats(filename, isBaseline, adaptive, smoothed):
    ...
    for i in range(repeat):
        ...
        res.append(rec)
    res = np.array(res).T
    if isBaseline or adaptive: return res
    return np.vstack((np.repeat(err_ls, repeat).reshape(1, -1), res))

def getTaxiRes(METHOD, EPS):
    dir = f"results/TaxiTrajectory/taxi_{EPS}/"
    num_curve = 0; stats = None
    for subdir in sorted(os.scandir(dir), key = lambda e: e.name):
        ...
        for file in os.listdir(dir+folder):
            if METHOD in file:
                num_curve += 1
                if stats is None: stats = getStats(dir+folder+file, False, True, True)
                else: stats = np.hstack((stats, getStats(dir+folder+file, False, True, True)))
    if not GET_MSE: return stats.mean(axis = 1)
    return ((stats-stats.mean(axis = 1, keepdims = True))**2).mean(axis = 1)
...
def getTaxiBLRes(METHOD, EPS, SAMPLE_RATE, WINDOW_SCALE):
    ...
    for subdir in sorted(os.scandir(dir), key = lambda e: e.name):
        ...
        for file in os.listdir(dir+folder):
            if METHOD in file:
                num_curve += 1
                if stats is None: stats = getStats(dir+folder+file, True, False, True)
                else: stats = np.hstack((stats, getStats(dir+folder+file, True, False, True)))
    if not GET_MSE: return stats.mean(axis = 1)
    return ((stats-stats.mean(axis = 1, keepdims = True))**2).mean(axis = 1)

def getECGRes(METHOD, EPS, BATCH_SIZE):
    dir = f"results/ECG/ECG_{EPS}_{BATCH_SIZE*TIME_SCALE//FREQUENCY}x{N//BATCH_SIZE}/"
    num_rec = 0; stats = None
    for i in range(22):
        ...
        for j in range(1000):
            ...
            if os.path.exists(dir+folder+file):
                num_rec += 1
                if stats is None: stats = getStats(dir+folder+file, False, False, True)
                else: stats = np.hstack((stats, getStats(dir+folder+file, False, False, True)))
    if not GET_MSE: return stats.mean(axis = 1)
    return ((stats-stats.mean(axis = 1, keepdims = True))**2).mean(axis = 1)
