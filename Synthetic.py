...
def plotEg(idx, method, eps = 0.1, plotBaseline = False, SAMPLE = 10):
    ...
    for idx, DEGREE in enumerate([1, 2, 4, 8]):
        ...
        solver.smooth()
        plt.plot(x_dense, solver.createPriv()(x_dense), color = 'tab:purple', alpha = 0.9, label = "privatization (continuous)")
        plt.title(f"Degree-{DEGREE} Polynomial Basis"); plt.legend(loc = 'upper left')
    plt.tight_layout()
    plt.show()
...
# plotEg(1, 'Gaussian', plotBaseline = True, SAMPLE = 100)
expt('Laplace')
expt('Gaussian')